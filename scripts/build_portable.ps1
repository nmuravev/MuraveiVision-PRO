...
if ($FetchEmbeddablePython) {
  ...
  if ($mirrorHost) {
    ...
    # MIRROR_EXCLUDE: dev-only packages (no runtime imports in backend/services)
    # R4 verified: no open3d/xformers imports in backend — these are depth_anything_3 optional deps
    $mirrorExcludeDirs = @("__pycache__", "dash", "flask", "ipywidgets", "jupyterlab_widgets", "widgetsnbextension",
