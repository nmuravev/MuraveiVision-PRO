...
export function useReconBuild(sourcePath: string | null | undefined, isAuthenticated: boolean) {
  ...
  const loadManifest = useCallback(async (): Promise<ReconManifest | null> => {
    ...
    const data = (await res.json()) as { manifest?: ReconManifest | null };
    ...
    (openSceneOnDone = false) => {
      ...
      void readSse(
        ...
        (data) => {
          ...
          setReconProgress(String(data.message || ''), progress, running);
          if (
            data.status === 'done' ||
        ...
        try {
          ...
          if (!stRes.ok) {
            setReconProgress('Поток SSE прерван', useReconStore.getState().reconProgress, false);
            return;
          }
  ...
  const startRecon = useCallback(
    async (opts?: {
      ...
      setReconProgress('Запуск реконструкции…', 0, true);
      try {
        ...
        setReconProgress(err instanceof Error ? err.message : 'Ошибка recon', 0, false);
      }
    },
    [sourcePath, setReconProgress, attachStream],
  );

  const stopRecon = useCallback(async () => {
    streamAbortRef.current?.abort();
    await fetch('/api/recon/stop', { method: 'POST', headers: authHeaders() });
    setReconProgress('Остановка…', useReconStore.getState().reconProgress, false);
  }, [setReconProgress]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const st = (await stRes.json()) as { status?: string; message?: string; progress?: number };
      if (st.status === 'running') {
        setReconProgress(st.message || 'Выполняется…', Number(st.progress ?? 0), true);
        attachStream(false);
      }
    })();
    return () => {
