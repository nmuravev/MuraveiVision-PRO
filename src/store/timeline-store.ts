...
export const useTimelineStore = create<TimelineState>()(
  subscribeWithSelector(
    ...
    setMediaDuration: (duration) => {
      set({ mediaDuration: Math.max(0, duration) });
    },
    ...
    seekTo: (position: number) => {
      ...
      const max = mediaDuration > 0 ? mediaDuration : Number.POSITIVE_INFINITY;
      const clampedPosition = Math.max(0, Math.min(max, position));
      set({
    ...
    stepFrame: (deltaFrames: number) => {
      ...
      const max = mediaDuration > 0 ? mediaDuration : Number.POSITIVE_INFINITY;
      const newPosition = Math.max(0, Math.min(max, playheadPosition + deltaFrames * FRAME_DURATION_SEC));
      set({
