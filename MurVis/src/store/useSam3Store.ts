...
// P0-10: Track all intervals for cleanup on store destruction
const _activeIntervals: Set<number> = new Set();
...
function stopPropPoll() {
  if (propPoll != null) {
    ...
    _activeIntervals.delete(propPoll);
    propPoll = null;
  }
}

// P0-10: Safe interval wrapper that tracks intervals for cleanup
function safeSetInterval(callback: () => void, delay: number): number {
  const id = setInterval(callback, delay) as unknown as number;
  _activeIntervals.add(id);
  return id;
}

// P0-10: Cleanup all active intervals
...
export const useSam3Store = create<Sam3State>((set, get) => ({
  ...
  startPropagate: async ({ videoPath, timeSec, maxFrames = 30, persist }) => {
    ...
    try {
      ...
      // P0-10: Use safeSetInterval for tracking
      propPoll = safeSetInterval(() => {
        void (async () => {
          try {
