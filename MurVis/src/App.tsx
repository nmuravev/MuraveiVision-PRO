...
function App() {
  ...
  const viewerSourceModes = useViewerStore((s) => {
    // Select only sourceMode values for the 4 viewers — avoids full object spread
    return {
      'viewer-1': s.viewers['viewer-1']?.sourceMode,
      'viewer-2': s.viewers['viewer-2']?.sourceMode,
      'viewer-3': s.viewers['viewer-3']?.sourceMode,
      'viewer-4': s.viewers['viewer-4']?.sourceMode,
    };
  });
  ...
  useEffect(() => {
    ...
    for (const id of ['viewer-1', 'viewer-2', 'viewer-3', 'viewer-4'] as const) {
      if (viewerSourceModes[id] !== 'live') setSourceMode(id, 'live');
    }
  }, [workspaceMode, viewerSourceModes, setSourceMode]);
