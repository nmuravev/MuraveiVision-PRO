...
export const Viewer: React.FC<ViewerProps> = ({ viewerId }) => {
  ...
  /** Stabilize WebSocket deps — always read latest values inside callbacks. */
  const focusedViewerIdRef = useRef(focusedViewerId);
  const sourceVideoRef = useRef(sourceVideo);
  const setLastObjectsRef = useRef(setLastObjects);
  const commitFrameRef = useRef(commitFrame);
  ...
  // Sync stabilized refs — always latest, never trigger re-renders
  focusedViewerIdRef.current = focusedViewerId;
  sourceVideoRef.current = sourceVideo;
  setLastObjectsRef.current = setLastObjects;
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (ev) => {
        ...
        try {
          ...
          setLiveStamp(stamp);
          const ruleMatches = useRulesStore.getState().evaluate(dets, sourceVideoRef.current, stamp);
          ...
          if (viewerId === focusedViewerIdRef.current && dets.length && !isSeekBlocked()) {
    ...
    viewerId,
    isAuthenticated,
    // Stabilized via refs — always read latest inside callbacks without
    // recreating the WebSocket when these values change:
    focusedViewerIdRef,
    sourceVideoRef,
    setLastObjectsRef,
    commitFrameRef,
