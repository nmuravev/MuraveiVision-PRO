...
export function useReconBuild(sourcePath: string | null | undefined, isAuthenticated: boolean) {
  ...
  const openSceneOnDoneRef = useRef(false);
      ...
      void readSse(
        ...
        (data) => {
          ...
          if (terminal) {
            ...
            void loadManifest();
            if (done && openSceneOnDoneRef.current) setViewMode('scene');
          }
        },
        abort,
      ).catch(async () => {
        ...
        try {
          ...
          if (done) {
            void loadManifest();
            if (openSceneOnDoneRef.current) setViewMode('scene');
          }
        } catch {
          setReconProgress('Поток SSE прерван', useReconStore.getState().reconProgress, false, null);
        }
  ...
  const startRecon = useCallback(
    async (opts?: {
      ...
      try {
        ...
        // Sync ref so nested callbacks always read latest UI value
        openSceneOnDoneRef.current = Boolean(opts?.openSceneOnDone);
        attachStream();
      } catch (err) {
