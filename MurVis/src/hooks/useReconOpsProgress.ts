...
export function useReconOpsProgress(opts: {
  ...
  const steps: OpsStep[] = useMemo(() => {
    // tick removed from dependencies — steps only change on real state events
    // (reconPhase, train.status, sceneLoading), not on 1s timer ticks.
    // The interval in useEffect still fires for UI animation if needed.
    const isTrain =
      opKindRef.current === 'train' ||
      training ||
