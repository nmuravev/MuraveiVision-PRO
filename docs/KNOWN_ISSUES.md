...
- **Ops progress modal log tail:** last **20** SSE/message lines only. COLMAP feature extract + mapper — один шаг UI (BE не шлёт раздельные phase).
- **Синие «кубики» на Сцене (2026-09-04):** это sparse COLMAP (`THREE.Points` + disc sprites; размер уменьшен для плотных облаков), **не** фотореализм. Фотореализм — после **Balanced/High** gsplat (`model.ply`). Bootstrap = минимальный splat, тоже не фотореализм. Idle-карточка на canvas объясняет pipeline и ведёт на Balanced. Модалка прогресса видна только во время COLMAP/train (после успеха закрывается).
- **Ops modal idle (2026-09-04):** после `colmap_done` модалка исчезает — это нормально; смотрите карточку «Готово: sparse…» на canvas, не считайте сцену «пустой». Тестируйте UI через Vite (`:3000`), не stale `dist`.
