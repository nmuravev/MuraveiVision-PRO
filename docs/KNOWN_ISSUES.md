...
- **DropInViewer placeholder bbox `[2,2,2]` (2026-09-04):** после `addSplatScene` Three.js иногда даёт фиктивный unit box. UI **не** dispose’ит splat и не откатывается на Points; кадрирует по Points/sparse и позже re-frame. Placeholder **не** пишется в `sceneError` (иначе ops modal → «Повторить»).
