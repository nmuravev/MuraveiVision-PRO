...
- `DA3_WEIGHTS_NOT_FOUND`: файлы весов `da3_{base,large,metric,giant}.safetensors` отсутствуют в `sidecars/da3/` (Mini без sidecar или FullKit без seed)
- `DA3_RUNTIME_UNAVAILABLE`: пакет `depth_anything_3` не импортируется / модель не загрузилась / инференс упал; плоский depth-fallback отключён (анти-мусорное облако-стена)

**Решения:**
- Загрузите модель (`POST /api/seg/load` или кнопка «Загрузить»)
- Подождите ~30 секунд и повторите
- Проверьте GPU / DA3: `GET /api/system/hardware` (`da3.*_present`)
- Для DA3: скопируйте веса в `sidecars/da3/` из FullKit (`scripts\fetch_da3_weights.py` + `stage_da3_sidecar.ps1`); Dense = DA3 (не AliceVision MVS)
- Для `DA3_RUNTIME_UNAVAILABLE`: установите `depth-anything-3` в `muravei_env` (+ wheel в `portable/cache/wheels` для air-gap bake FullKit)
