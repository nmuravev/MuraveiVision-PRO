...
AI-анализ **не требует** Ollama при старте — спокойный статус «отключена». Подключение явное (AI-анализ → панель Ollama).

| Сценарий | Как |
|----------|-----|
| Same OS | «Подключить» — лестница: `MURAVEI_OLLAMA_URL` → `127.0.0.1` / `[::1]` → WSL IP → сохранённый URL |
| WSL → Windows | Backend в WSL находит gateway (хост Windows):11434 |
| Windows → WSL | Backend на Windows читает IP инстанса через `wsl -- ip …` |
| LAN | Кнопка «Найти в сети» (только по клику; не на старте) |
| Вручную | Host/port в форме → «Сохранить и подключить»; или env `MURAVEI_OLLAMA_URL` |

Конфиг: `config/local/ollama.json` (gitignored). Автоподключение при старте backend — в фоне, wall-clock **≤5 с**, без блокировки API.

Если порт 11434 занят чужим сервисом: сообщение «Порт 11434 занят другим сервисом, проверьте MURAVEI_OLLAMA_URL».
